...
export default function DocumentsPage() {
  ...
  const handleUpload = () => {
    toast({
      ...
      description: "Document uploading will be fully integrated in the next sprint."
    });
  };
        ...
        <Card className="bg-white dark:bg-zinc-900 border-zinc-200/60 dark:border-zinc-800/60 shadow-xs rounded-2xl overflow-hidden">
          <CardContent className="p-8 text-center space-y-6">
            ...
            {loading ? (
              ...
              <div className="space-y-4 py-8">
                <div className="space-y-1.5">
                  <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200">No documents uploaded yet.</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-xs mx-auto">
                    Upload notes, PDFs and lecture materials.
                  </p>
                </div>
                <div className="pt-2">
                  <Button
                    onClick={handleUpload}
                    className="bg-primary hover:bg-indigo-500 text-white font-bold text-xs px-6 py-2.5 rounded-xl shadow-xs cursor-pointer transition-all active:scale-98 inline-flex items-center gap-2"
                  >
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100/50 dark:border-indigo-900/30 text-xs font-bold text-primary dark:text-indigo-300">
              ...
              <span>Documents System Active</span>
            </div>
